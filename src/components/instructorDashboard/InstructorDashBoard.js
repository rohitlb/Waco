...
class InstructorDashboard extends Component {
    ...
    async componentDidMount(){
        ...
        if(!data) this.setState({user:null})
            ...
            const result=await post('find-instructor',token,{email})
            console.log(result)
    ...
    Object.assign(submitObject,{imageUrl})
    console.log(submitObject)
    if(this.passwordValidate()){
    const result= await post('instructor',this.state.user.token,submitObject);
    if(result.status)
...
passwordValidate=()=>{
    ...
    render(){
        const {user,other}=this.state
        if(user && other){
            const {email,name,imageUrl,token}=user
                    ...
                    <button className="navbar-toggler navbar-toggler-right" type="button" data-toggle="collapse" data-target="#navbarToggler-1" aria-controls="navbarToggler-1" aria-expanded="false" aria-label="Toggle navigation">
                       <span className="navbar-toggler-icon"></span>
                        ...
                        <NavLink to ="/edusmithMain" className="navbar-brand"  style={{ margin: "auto" }}><img alt="" src={"https://res.cloudinary.com/drcx4jfun/image/upload/v1597165109/final_try_logo_fj9vsw.png"} style={{ height: "70px", marginLeft: "15px" }} /></NavLink>
                                    ...
                                    <NavLink to="/edusmith/InstructorDashboard" className="nav-link" style={{ color: "#fff", fontSize: "2rem", textShadow: "2px 2px 2px black" }}>Your Dashboard</NavLink>
                                                </li>
                                                <li className="nav-item">
